...
function npcsay(id)
  ...
  if qData[1071].state == 1 and qData[1071].killMonster[qt[1071].goal.killMonster[1].id] >= qt[1071].goal.killMonster[1].count then
    if __QUEST_HAS_ALL_ITEMS(qt[1071].goal.getItem) and __QUEST_HAS_ALL_ITEMS(qt[1071].goal.getItem) then
      NPC_SAY("?? ??? ? ?? ?? ???? ??? ??? ? ????.")
  ...
  if qData[1252].state == 1 and __QUEST_HAS_ALL_ITEMS(qt[1252].goal.getItem) and __QUEST_HAS_ALL_ITEMS(qt[1252].goal.getItem) then
