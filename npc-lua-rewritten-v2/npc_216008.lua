...
function npcsay(id)
  ...
  if qData[2439].state == 1 then
    if __QUEST_HAS_ALL_ITEMS(qt[2439].goal.getItem) and __QUEST_HAS_ALL_ITEMS(qt[2439].goal.getItem) then
      NPC_SAY("°¨»çÇÕ´Ï´Ù.")
  ...
  if qData[2492].state == 1 then
    if __QUEST_HAS_ALL_ITEMS(qt[2492].goal.getItem) and __QUEST_HAS_ALL_ITEMS(qt[2492].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(4) then
        NPC_SAY("¾îµð.. ¸Â±º¿ä.")
