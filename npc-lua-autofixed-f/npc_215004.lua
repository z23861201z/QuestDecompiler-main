function npcsay(id)
  ...
  if qData[276].state == 1 then
    if __QUEST_HAS_ALL_ITEMS(qt[276].goal.getItem) and __QUEST_HAS_ALL_ITEMS(qt[276].goal.getItem) then
      NPC_SAY("?? ??? ? ?? ??? ? ??? ????.")
  ...
  if qData[1310].state == 1 then
    if __QUEST_HAS_ALL_ITEMS(qt[1310].goal.getItem) and __QUEST_HAS_ALL_ITEMS(qt[1310].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(3) then
