local function __QUEST_HAS_ALL_KILL_MONSTER(killData, killGoals)
  for i, v in ipairs(killGoals) do
    local current = 0
    if killData ~= nil then
      current = killData[v.id] or 0
    end
...
function npcsay(id)
  ...
  if qData[1557].state == 1 then
    if CHECK_ITEM_CNT(qt[1557].goal.getItem[1].id) >= qt[1557].goal.getItem[1].count and CHECK_ITEM_CNT(qt[1557].goal.getItem[2].id) >= qt[1557].goal.getItem[2].count and __QUEST_HAS_ALL_KILL_MONSTER(qData[1557].killMonster, qt[1557].goal.killMonster) then
