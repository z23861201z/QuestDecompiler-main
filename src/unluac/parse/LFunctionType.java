...
public class LFunctionType extends BObjectType<LFunction> {
  ...
  public LFunction parse(ByteBuffer buffer, BHeader header) {
    ...
    lfunc.source = s.name;
    lfunc.lineBegin = s.lineBegin;
    lfunc.lineEnd = s.lineEnd;
    lfunc.lines = new int[s.lines.length.asInt()];
    for(int i = 0; i < lfunc.lines.length; i++) {
      lfunc.lines[i] = s.lines.get(i).asInt();
    }
