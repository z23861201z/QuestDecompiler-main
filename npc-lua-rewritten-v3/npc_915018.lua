...
function npcsay(id)
  ...
  if qData[142].state == 1 then
    if __QUEST_HAS_ALL_ITEMS(qt[142].goal.getItem) and __QUEST_HAS_ALL_ITEMS(qt[142].goal.getItem) then
...
?? ?? ???. ?? ?? ???!]])
  ...
  if qData[1069].state == 1 then
    if __QUEST_HAS_ALL_ITEMS(qt[1069].goal.getItem) and __QUEST_HAS_ALL_ITEMS(qt[1069].goal.getItem) then
      NPC_SAY("?? ????? ? ?? ??? ?? ????. ?? ?? ?? ???.")
