...
function npcsay(id)
  ...
  if qData[2330].state == 1 and __QUEST_HAS_ALL_ITEMS(qt[2330].goal.getItem) and __QUEST_HAS_ALL_ITEMS(qt[2330].goal.getItem) then
    NPC_SAY("爷爷？初次见面")
...
function chkQState(id)
  ...
  if qData[2330].state == 1 then
    if __QUEST_HAS_ALL_ITEMS(qt[2330].goal.getItem) and __QUEST_HAS_ALL_ITEMS(qt[2330].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
