...
- **主框架**：Spring Boot 3.x（本项目使用 JDK 1.8 + Maven 构建）
- **语言版本**：Java 1.8
- **构建工具**：Maven
- **核心依赖**：
...
> 注：该目录结构基于当前工作区路径 `D:\github\QuestDecompiler-main`，所有源码位于 `src/orgunluac` 下，测试代码位于 `test/src` 目录中。
...
- 项目由作者 **JIJIA** 编写，后续应保持一致的编码风格和设计思路。
- 若引入第三方 SDK 或模块，请确保其兼容性并遵守对应的许可协议。
- 测试覆盖率应不低于 80%，且关键功能必须具备单元测试或集成测试支持。
