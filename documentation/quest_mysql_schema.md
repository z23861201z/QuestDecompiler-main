...
## 当前代码落地（ghost_game）增量补全说明（2026-02-12）

说明：上文为目标型通用设计；当前项目 Phase3 实际写入库为 `ghost_game`，本次按“补全而非重建”原则做了增量扩展。
...
用途：
- 兜底保存 `reward/goal/conditions` 扩展语义，避免 `DB -> quest.lua/luc` 回导时字段丢失。

### 2) 新增表 `quest_reward_skill`
- 字段：`quest_id`, `seq_index`, `skill_id`
- 索引：
  - `UNIQUE KEY uk_reward_skill (quest_id, seq_index)`
  - `KEY idx_reward_skill_quest (quest_id)`

用途：
- 保留 `reward.getSkill[]` 顺序，支持可逆导出。

### 3) 迁移脚本与回滚脚本
- 迁移：`documentation/sql/phase2_4_field_completion_migration.sql`
- 回滚：`documentation/sql/phase2_4_field_completion_rollback.sql`
...
- Phase3 校验：`src/unluac/semantic/Phase3DbConsistencyValidator.java`
- Phase4 导出：`src/unluac/semantic/Phase4QuestLucExporter.java`
- Phase4 校验：`src/unluac/semantic/Phase4QuestExportValidator.java`
