function npcsay(id)
  ...
  if qData[2675].state == 1 then
    if __QUEST_HAS_ALL_ITEMS(qt[2675].goal.getItem) and __QUEST_HAS_ALL_KILL_TARGETS(qt[2675].goal.killMonster) then
      NPC_SAY("谢谢，太感谢了。")
  ...
  if qData[2676].state == 1 then
    if CHECK_ITEM_CNT(qt[2676].goal.getItem[1].id) >= qt[2676].goal.getItem[1].count and __QUEST_HAS_ALL_KILL_TARGETS(qt[2676].goal.killMonster) then
      NPC_SAY("真心感谢！")
