...
function npcsay(id)
  ...
  if qData[1270].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1270].goal.getItem) then
  ...
  if qData[1043].state == 1 and __QUEST_CHECK_ITEMS(qt[1043].goal.getItem) then
  ...
  if qData[1271].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1271].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(3) then
...
function chkQState(id)
  ...
  if qData[1043].state == 1 and GET_PLAYER_LEVEL() >= qt[1043].needLevel then
    if __QUEST_CHECK_ITEMS(qt[1043].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
