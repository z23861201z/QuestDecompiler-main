...
function npcsay(id)
  ...
  if qData[153].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[153].goal.getItem) then
  ...
  if qData[1281].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1281].goal.getItem) then
  ...
  if qData[1095].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1095].goal.getItem) then
...
function chkQState(id)
  ...
  if qData[1087].state == 1 and GET_PLAYER_LEVEL() >= qt[1087].needLevel then
    if __QUEST_CHECK_ITEMS(qt[1087].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
