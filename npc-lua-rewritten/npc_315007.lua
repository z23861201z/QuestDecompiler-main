...
function npcsay(id)
  ...
  if qData[235].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[235].goal.getItem) then
      NPC_SAY("?? ??? ????. ??? ????? ??? ??? ??? ????? ??? ?? ??? ??? ?? ??? ????. ?? ????? ????.")
  ...
  if qData[240].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[240].goal.getItem) then
      NPC_SAY("?? ??? ??????. ?? ??? ???? ???? ??? ????. ??? ? ???? ? ?????? ??? ??? ?? ?? ??.")
  ...
  if qData[244].state == 1 and qData[242].state == 2 and __QUEST_CHECK_ITEMS(qt[244].goal.getItem) then
    NPC_SAY("??? ??? ?? ????? ??? ??? ?? ???? ??? ?????")
  ...
  if qData[473].state == 1 and __QUEST_CHECK_ITEMS(qt[473].goal.getItem) then
    NPC_QSAY(473, 1)
  ...
  if qData[1071].state == 1 and qData[1071].killMonster[qt[1071].goal.killMonster[1].id] >= qt[1071].goal.killMonster[1].count then
    if __QUEST_CHECK_ITEMS(qt[1071].goal.getItem) and __QUEST_CHECK_ITEMS(qt[1071].goal.getItem) then
      NPC_SAY("?? ??? ? ?? ?? ???? ??? ??? ? ????.")
  ...
  if qData[1252].state == 1 and __QUEST_CHECK_ITEMS(qt[1252].goal.getItem) and __QUEST_CHECK_ITEMS(qt[1252].goal.getItem) then
...
function chkQState(id)
  ...
  if qData[244].state == 1 and qData[242].state == 2 and GET_PLAYER_LEVEL() >= qt[244].needLevel then
    if __QUEST_CHECK_ITEMS(qt[244].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
  ...
  if qData[473].state == 1 and GET_PLAYER_LEVEL() >= qt[472].needLevel then
    if __QUEST_CHECK_ITEMS(qt[473].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
