...
function npcsay(id)
  ...
  clickNPCid = id
  if qData[1008].state == 1 and __QUEST_CHECK_ITEMS(qt[1008].goal.getItem) then
  ...
  if qData[1289].state == 1 and __QUEST_CHECK_ITEMS(qt[1289].goal.getItem) then
    NPC_SAY("嗯？是陈调寄来的命令书？")
...
function chkQState(id)
  ...
  if qData[1008].state == 1 and GET_PLAYER_LEVEL() >= qt[1008].needLevel then
    if __QUEST_CHECK_ITEMS(qt[1008].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
