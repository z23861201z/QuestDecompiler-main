...
function npcsay(id)
  ...
  if qData[234].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[234].goal.getItem) then
      NPC_SAY("??????. ???? ??? ? ?? ??? ????. ?? ??????. ??? ??? ?????. ???? ?? ??? ? ??????.")
  ...
  if qData[544].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[544].goal.getItem) then
      NPC_SAY("?? ??????. ??? ???? ?? ? ? ????.")
  ...
  if qData[545].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[545].goal.getItem) then
      NPC_SAY("???????! ??????. ? ?? ???? ??????.")
  ...
  if qData[557].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[557].goal.getItem) then
      NPC_SAY("?????. PLAYERNAME? ????? ???? ? ??? ??? ????, ?? ??? ? ?????.")
  ...
  if qData[1070].state == 1 then
    if qData[1070].killMonster[qt[1070].goal.killMonster[1].id] >= qt[1070].goal.killMonster[1].count and __QUEST_CHECK_ITEMS(qt[1070].goal.getItem) then
      NPC_SAY("??? ?????? ??? ??????. ?????.")
  ...
  if qData[1284].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1284].goal.getItem) then
  ...
  if qData[1303].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1303].goal.getItem) then
  ...
  if qData[1304].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1304].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(3) then
        NPC_SAY("辛苦了。现在开始试试吧。")
