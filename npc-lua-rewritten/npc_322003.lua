...
function npcsay(id)
  ...
  if qData[2331].state == 1 and __QUEST_CHECK_ITEMS(qt[2331].goal.getItem) then
  ...
  if qData[2677].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2677].goal.getItem) then
      NPC_SAY("这么快，太谢谢了！")
...
function chkQState(id)
  ...
  if qData[2331].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2331].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
  ...
  if qData[2792].state == 1 and __QUEST_CHECK_ITEMS(qt[2792].goal.getItem) and qData[2792].killMonster[qt[2792].goal.killMonster[1].id] >= qt[2792].goal.killMonster[1].count then
    QSTATE(id, 2)
  end
