...
function npcsay(id)
  ...
  if qData[142].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[142].goal.getItem) and __QUEST_CHECK_ITEMS(qt[142].goal.getItem) then
...
?? ?? ???. ?? ?? ???!]])
  ...
  if qData[162].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[162].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(2) then
        NPC_SAY("?? ?? ? ??? ????. ?? ??? ????.")
  ...
  if qData[1069].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1069].goal.getItem) and __QUEST_CHECK_ITEMS(qt[1069].goal.getItem) then
      NPC_SAY("?? ????? ? ?? ??? ?? ????. ?? ?? ?? ???.")
  ...
  if qData[238].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[238].goal.getItem) then
      NPC_SAY("???????. ?? ??? ??? ? ??? ?????. ??? ???? ??? ??? ?????. ????. ?? ??? ??? ????.")
