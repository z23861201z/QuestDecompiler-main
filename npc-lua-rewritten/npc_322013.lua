...
function npcsay(id)
  ...
  if qData[2671].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2671].goal.getItem) then
      NPC_SAY("这么快！")
  ...
  if qData[2672].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2672].goal.getItem) then
  ...
  if qData[2673].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2673].goal.getItem) and __QUEST_CHECK_ITEMS(qt[2673].goal.getItem) then
      NPC_SAY("正等你呢，再等会儿就好。")
