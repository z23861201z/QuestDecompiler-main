...
function npcsay(id)
  ...
  if qData[791].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[791].goal.getItem) then
      NPC_SAY("{0xFFFFFF00}????{END}? ?? ??????? ?? ??????.")
  ...
  if qData[793].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[793].goal.getItem) then
      NPC_SAY("??? ??? ???? ??????. ?? ??? ?? ?????.")
  ...
  if qData[794].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[794].goal.getItem) then
      NPC_SAY("??? ??? ???? ??????. ?? ??? ?? ?????.")
  ...
  if qData[795].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[795].goal.getItem) then
      NPC_SAY("??? ??? ???? ??????. ?? ??? ?? ?????.")
