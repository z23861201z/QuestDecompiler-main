...
function npcsay(id)
  ...
  clickNPCid = id
  if qData[136].state == 1 and __QUEST_CHECK_ITEMS(qt[136].goal.getItem) then
  ...
  if qData[242].state == 1 and qData[241].state == 2 and qData[243].state == 2 and __QUEST_CHECK_ITEMS(qt[242].goal.getItem) then
    if 1 <= CHECK_INVENTORY_CNT(2) then
  ...
  if qData[456].state == 1 and qData[455].state == 2 then
    if __QUEST_CHECK_ITEMS(qt[456].goal.getItem) then
      NPC_QSAY(456, 3)
  ...
  if qData[1241].state == 1 and __QUEST_CHECK_ITEMS(qt[1241].goal.getItem) then
  ...
  if qData[1242].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1242].goal.getItem) then
  ...
  if qData[1243].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1243].goal.getItem) then
      NPC_SAY("希望这些能让她满意。")
  ...
  if qData[1244].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1244].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(2) then
...
function chkQState(id)
  ...
  if qData[136].state == 1 and GET_PLAYER_LEVEL() >= qt[136].needLevel then
    if __QUEST_CHECK_ITEMS(qt[136].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
  ...
  if qData[1241].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1241].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(4) then
        QSTATE(id, 2)
      else
        QSTATE(id, 1)
