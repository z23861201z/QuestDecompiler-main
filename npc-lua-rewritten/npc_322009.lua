...
function npcsay(id)
  ...
  if qData[2330].state == 1 and __QUEST_CHECK_ITEMS(qt[2330].goal.getItem) and __QUEST_CHECK_ITEMS(qt[2330].goal.getItem) then
    NPC_SAY("爷爷？初次见面")
...
function chkQState(id)
  ...
  if qData[2330].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2330].goal.getItem) and __QUEST_CHECK_ITEMS(qt[2330].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
  ...
  if qData[2791].state == 1 and __QUEST_CHECK_ITEMS(qt[2791].goal.getItem) and qData[2791].killMonster[qt[2791].goal.killMonster[1].id] >= qt[2791].goal.killMonster[1].count then
    QSTATE(id, 2)
  end
