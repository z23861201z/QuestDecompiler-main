...
function npcsay(id)
  ...
  if qData[84].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[84].goal.getItem) then
      NPC_SAY("???? ?? ????. ??? ?????.. ??? ????? ???? ??? ? ??? ?? ????. ?????. ?? ??? ??????.")
  ...
  if qData[122].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[122].goal.getItem) and __QUEST_CHECK_ITEMS(qt[122].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(4) then
  ...
  if qData[127].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[127].goal.getItem) and __QUEST_CHECK_ITEMS(qt[127].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(4) then
  ...
  if qData[154].state == 1 and qData[154].meetNpc[1] ~= id and __QUEST_CHECK_ITEMS(qt[154].goal.getItem) then
  ...
  if qData[201].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[201].goal.getItem) then
      NPC_SAY("?????. ?????. ???? ??? ??? ?? ? ????. ????? ??? ??? ??? ??? ????. ?? ?????.")
  ...
  if qData[339].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[339].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(4) then
        NPC_SAY("?, ?? ???????. ?, ????? ?? ?? ????? ?????? ? ?? ??? ?????.")
  ...
  if qData[342].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[342].goal.getItem) and __QUEST_CHECK_ITEMS(qt[342].goal.getItem) and __QUEST_CHECK_ITEMS(qt[342].goal.getItem) and __QUEST_CHECK_ITEMS(qt[342].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(4) then
        NPC_SAY("?~ ??? ??????. ??? ?????…. ?! ??????.")
  ...
  if qData[348].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[348].goal.getItem) then
  ...
  if qData[548].state == 1 and __QUEST_CHECK_ITEMS(qt[548].goal.getItem) and __QUEST_CHECK_ITEMS(qt[548].goal.getItem) then
    NPC_SAY("?? ??? ??????")
    SET_QUEST_STATE(548, 2)
  end
  if qData[549].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[549].goal.getItem) and __QUEST_CHECK_ITEMS(qt[549].goal.getItem) then
      NPC_SAY("???? ?? ???. ?? ????.")
