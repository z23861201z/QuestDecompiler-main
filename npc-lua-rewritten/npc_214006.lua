...
function npcsay(id)
  ...
  if qData[1150].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1150].goal.getItem) then
  ...
  if qData[1183].state == 1 and __QUEST_CHECK_ITEMS(qt[1183].goal.getItem) then
    if 1 <= CHECK_INVENTORY_CNT(3) then
  ...
  if qData[1184].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1184].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(3) then
        NPC_SAY("真是太感谢了，你一定会有好报的。")
  ...
  if qData[1187].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1187].goal.getItem) then
...
function chkQState(id)
  ...
  if qData[1183].state ~= 2 and GET_PLAYER_LEVEL() >= qt[1183].needLevel and qData[1183].state == 1 and __QUEST_CHECK_ITEMS(qt[1183].goal.getItem) then
    QSTATE(id, 2)
  end
