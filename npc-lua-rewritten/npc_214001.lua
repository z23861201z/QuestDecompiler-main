...
function npcsay(id)
  ...
  if qData[1113].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1113].goal.getItem) then
  ...
  if qData[1341].state == 1 and __QUEST_CHECK_ITEMS(qt[1341].goal.getItem) then
    if 1 <= CHECK_INVENTORY_CNT(3) then
...
function chkQState(id)
  ...
  if qData[1341].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1341].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
