...
function npcsay(id)
  ...
  if qData[1483].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1483].goal.getItem) then
  ...
  if qData[874].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[874].goal.getItem) and __QUEST_CHECK_ITEMS(qt[874].goal.getItem) and __QUEST_CHECK_ITEMS(qt[874].goal.getItem) then
  ...
  if qData[875].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[875].goal.getItem) and __QUEST_CHECK_ITEMS(qt[875].goal.getItem) and __QUEST_CHECK_ITEMS(qt[875].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(3) then
        NPC_SAY("辛苦了。这是{0xFFFFFF00}近卫兵可心{END}送来的奖励。那明天见吧！")
  ...
  if qData[2858].state == 1 and __QUEST_CHECK_ITEMS(qt[2858].goal.getItem) then
    NPC_SAY("这是什么味道啊？这不是{0xFFFFFF00}女儿红{END}吗？什么？是辛巴达介绍来的？")
    SET_QUEST_STATE(2858, 2)
    return
  end
  if qData[2859].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2859].goal.getItem) and __QUEST_CHECK_ITEMS(qt[2859].goal.getItem) and __QUEST_CHECK_ITEMS(qt[2859].goal.getItem) then
      NPC_SAY("哇！这就是新的酒！~")
  ...
  if qData[2860].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2860].goal.getItem) then
  ...
  if qData[2864].state == 1 and __QUEST_CHECK_ITEMS(qt[2864].goal.getItem) then
...
function chkQState(id)
  ...
  if qData[2858].state ~= 2 and qData[2857].state == 2 and GET_PLAYER_LEVEL() >= qt[2858].needLevel then
    if qData[2858].state == 1 then
      if __QUEST_CHECK_ITEMS(qt[2858].goal.getItem) then
        QSTATE(id, 2)
      else
        QSTATE(id, 1)
      end
  ...
  if qData[2864].state ~= 2 and qData[2863].state == 2 and GET_PLAYER_LEVEL() >= qt[2864].needLevel then
    if qData[2864].state == 1 then
      if __QUEST_CHECK_ITEMS(qt[2864].goal.getItem) then
        QSTATE(id, 2)
      else
        QSTATE(id, 1)
      end
