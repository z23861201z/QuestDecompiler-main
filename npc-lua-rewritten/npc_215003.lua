...
function npcsay(id)
  ...
  if qData[152].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[152].goal.getItem) then
  ...
  if qData[165].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[165].goal.getItem) then
      NPC_SAY("??! ?? ???~ ?? ?? ??? ???? ?? ?? ?? ? ????. ???? ???. ?? ??? ?????.")
  ...
  if qData[1273].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1273].goal.getItem) then
  ...
  if qData[1275].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1275].goal.getItem) and __QUEST_CHECK_ITEMS(qt[1275].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(3) then
