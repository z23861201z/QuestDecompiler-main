...
function npcsay(id)
  ...
  if qData[340].state == 1 and __QUEST_CHECK_ITEMS(qt[340].goal.getItem) then
    NPC_SAY("?? ?? ?? ????. ?????? ?????? ?? ?????.")
  ...
  if qData[343].state == 1 and __QUEST_CHECK_ITEMS(qt[343].goal.getItem) then
    NPC_SAY("?? ?? ?? ????. ?????? ?????? ?? ?????.")
  ...
  if qData[1224].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1224].goal.getItem) then
  ...
  if qData[1237].state == 1 and __QUEST_CHECK_ITEMS(qt[1237].goal.getItem) then
...
function chkQState(id)
  ...
  if qData[1224].state == 1 and __QUEST_CHECK_ITEMS(qt[1224].goal.getItem) then
    if CHECK_INVENTORY_CNT(3) > 0 then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
  ...
  if qData[340].state == 1 and GET_PLAYER_LEVEL() >= qt[340].needLevel then
    if __QUEST_CHECK_ITEMS(qt[340].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
  ...
  if qData[343].state == 1 and GET_PLAYER_LEVEL() >= qt[343].needLevel then
    if __QUEST_CHECK_ITEMS(qt[343].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
