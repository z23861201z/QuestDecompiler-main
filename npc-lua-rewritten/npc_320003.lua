...
function npcsay(id)
  ...
  clickNPCid = id
  NPC_SAY("ÄãÖªµÀÂð£¿¹íõþµºÓÐºÜ¶à¹ÊÊÂµÄ")
  ...
  if qData[2472].state == 1 then
    if 1 <= CHECK_INVENTORY_CNT(3) then
      NPC_SAY("Àú´Â ³ªÁß¿¡ ¿©ÇàÇÑ ÀÌ¾ß±â¸¦ Ã¥À¸·Î ¸¸µé »ý°¢ÀÔ´Ï´Ù. ±×¶§ ´ç½ÅÀÇ ÀÌ¾ß±â¸¦ Ã¥¿¡ ³Ö¾îµµ µÉ±î¿ä?(¸»ÀÌ ±æ¾îÁú °Í °°´Ù. 144°ø·ÂÀÌ µÈ ´ÙÀ½¿¡ ´Ù½Ã ¿ÀÀÚ.)")
  ...
  if qData[2473].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2473].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(3) then
      ...
      NPC_SAY("{0xFFFFFF00}Èæ±ÍÁÖ¿ø{END}À¸·Î °¡¼­ {0xFFFFFF00}[»ï°­µ¿Àü]{END}À» ÅðÄ¡ÇÏ°í {0xFFFFFF00}»ï°­µ¿Àüµî»Ô{END}À» 85°³±¸ÇØÁÖ¼¼¿ä.")
    end
  end
  if qData[2474].state == 1 then
    if qData[2474].killMonster[qt[2474].goal.killMonster[1].id] >= qt[2474].goal.killMonster[1].count then
      if 1 <= CHECK_INVENTORY_CNT(3) then
        NPC_SAY("À¯°ïÃþ±ÍÀÇ Æ¯Â¡À» Á» ¾Ë·ÁÁÖ¼¼¿ä.")
      ...
      NPC_SAY("{0xFFFFFF00}Èæ±ÍÁÖ¿ø{END}À¸·Î °¡¼­ {0xFFFFFF00}[À¯°ïÃþ±Í]{END}¸¦ 140¸¶¸® ÅðÄ¡ÇÏ°í ±×µéÀÇ Æ¯Â¡À» ¾Ë·ÁÁÖ¼¼¿ä.")
    end
  end
  if qData[2475].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2475].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(3) then
        NPC_SAY("°¨»çÇÕ´Ï´Ù. ÁÁÀº ÀÚ·á°¡ µÉ °ÍÀÔ´Ï´Ù.")
      ...
      NPC_SAY("{0xFFFFFF00}Èæ±ÍÁÖ¿ø{END}À¸·Î °¡¼­ {0xFFFFFF00}[À¯°ïÃþ±Í]{END}¸¦ ÅðÄ¡ÇÏ°í {0xFFFFFF00}ºÎ½º·¯ÁøÆÄÆí{END}À» 70°³±¸ÇØÁÖ¼¼¿ä.")
    end
  end
  if qData[2476].state == 1 then
    if qData[2476].killMonster[qt[2476].goal.killMonster[1].id] >= qt[2476].goal.killMonster[1].count then
      ...
      NPC_SAY("{0xFFFFFF00}Èæ±ÍÁÖ¿ø{END}À¸·Î °¡¼­ {0xFFFFFF00}[À°¾È±Í]{END}¸¦ 150¸¶¸® ÅðÄ¡ÇØÁÖ¼¼¿ä. ±× Á¤µµ¸é ÃæºÐÇÕ´Ï´Ù.")
    end
  end
  if qData[3678].state == 1 then
    if qData[3678].killMonster[qt[3678].goal.killMonster[1].id] >= qt[3678].goal.killMonster[1].count then
      ...
      NPC_SAY("»÷ÍË{0xFFFFFF00}50¸öºÚ¹íõþÔ´µÄÁùÑÛ¹Ö{END}°É")
    end
  end
  if qData[3682].state == 1 then
    if qData[3682].killMonster[qt[3682].goal.killMonster[1].id] >= qt[3682].goal.killMonster[1].count then
      ...
      NPC_SAY("»÷ÍË{0xFFFFFF00}50¸öºÚ¹íõþÔ´µÄ¶«À´³àÉ«¹í{END}°É")
    end
  end
