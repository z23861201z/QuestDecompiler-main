...
function npcsay(id)
  ...
  if qData[1409].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1409].goal.getItem) then
      NPC_SAY("????. ????? ???? ?? ?? ????. ??? ?? ?? ?? ?? ??? 2?? ??? ??? ?? ?? ??.")
  ...
  if qData[1410].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1410].goal.getItem) then
      NPC_SAY("????. ????? ???? ?? ?? ????. ??? ?? ?? ?? ?? ??? 2?? ??? ??? ?? ?? ??.")
  ...
  if qData[1411].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1411].goal.getItem) and __QUEST_CHECK_ITEMS(qt[1411].goal.getItem) then
      NPC_SAY("????. ????? ????? ??? ?? ??? ???. ??? ???? ???? ?? ?? ? ?? ???. ?? ?? ???.")
