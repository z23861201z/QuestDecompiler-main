...
function npcsay(id)
  ...
  if qData[347].state == 1 then
    if qData[347].killMonster[qt[347].goal.killMonster[1].id] >= qt[347].goal.killMonster[1].count and __QUEST_CHECK_ITEMS(qt[347].goal.getItem) then
      NPC_SAY("?? ?????. ?? ?? ????. ?? ?????.")
  ...
  if qData[1156].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1156].goal.getItem) then
  ...
  if qData[1163].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1163].goal.getItem) then
