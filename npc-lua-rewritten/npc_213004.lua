...
function npcsay(id)
  ...
  if qData[441].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[441].goal.getItem) and __QUEST_CHECK_ITEMS(qt[441].goal.getItem) then
  ...
  if qData[197].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[197].goal.getItem) then
      NPC_SAY("??? ? ???? ??? ?? ??????. ?? ?? ?? ?? ?? ?? ???. ?? ???? ??????.")
  ...
  if qData[554].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[554].goal.getItem) then
      NPC_SAY("??????~ ?? ?? ??? ??? ? ?? ??????. ???? ?? ??? ???? PLAYERNAME??? ? ?????????~")
