...
function npcsay(id)
  ...
  if qData[1476].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1476].goal.getItem) then
  ...
  if qData[1477].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1477].goal.getItem) then
  ...
  if qData[2593].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2593].goal.getItem) then
      NPC_SAY("谢谢。这是给你的奖励。")
  ...
  if qData[2880].state == 1 and __QUEST_CHECK_ITEMS(qt[2880].goal.getItem) then
    NPC_SAY("能快点吗？拜托了。")
    SET_QUEST_STATE(2880, 2)
    return
  end
  if qData[2888].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2888].goal.getItem) then
      NPC_SAY("谢谢。")
  ...
  if qData[2889].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2889].goal.getItem) then
      NPC_SAY("谢谢，这下能安心了。")
...
function chkQState(id)
  ...
  if qData[2880].state ~= 2 and qData[2879].state == 2 and GET_PLAYER_LEVEL() >= qt[2880].needLevel then
    if qData[2880].state == 1 then
      if __QUEST_CHECK_ITEMS(qt[2880].goal.getItem) then
        QSTATE(id, 2)
      else
        QSTATE(id, 1)
      end
