...
function npcsay(id)
  ...
  if qData[1132].state == 1 and __QUEST_CHECK_ITEMS(qt[1132].goal.getItem) then
    NPC_SAY("嗯？谁啊！")
    SET_QUEST_STATE(1132, 2)
  end
  if qData[1133].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1133].goal.getItem) and GET_SEAL_BOX_SOUL_PERSENT(8510011) >= 100 then
  ...
  if qData[2025].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2025].goal.getItem) then
  ...
  if qData[2065].state == 1 and __QUEST_CHECK_ITEMS(qt[2065].goal.getItem) then
...
function chkQState(id)
  ...
  if qData[1132].state ~= 2 and GET_PLAYER_LEVEL() >= qt[1132].needLevel and qData[1132].state == 1 and __QUEST_CHECK_ITEMS(qt[1132].goal.getItem) then
    QSTATE(id, 2)
  end
  if qData[1132].state == 2 and qData[1133].state ~= 2 and GET_PLAYER_LEVEL() >= qt[1133].needLevel then
    if qData[1133].state == 1 then
      if __QUEST_CHECK_ITEMS(qt[1133].goal.getItem) and GET_SEAL_BOX_SOUL_PERSENT(qt[1133].goal.getItem[1].id) >= 100 then
        QSTATE(id, 2)
      else
        QSTATE(id, 1)
      end
  ...
  if qData[2065].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2065].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
