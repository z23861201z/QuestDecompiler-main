...
function npcsay(id)
  ...
  clickNPCid = id
  NPC_SAY("ÏÖÔÚËäÈ»ÈõÐ¡£¬µ«ÊÇÎÒ±Ø¶¨»á±äµÃÇ¿´óÆðÀ´µÄ")
  ...
  if qData[2479].state == 1 then
    if qData[2479].killMonster[qt[2479].goal.killMonster[1].id] >= qt[2479].goal.killMonster[1].count then
      if 1 <= CHECK_INVENTORY_CNT(3) then
        NPC_SAY("¹ú½á 130¸¶¸®¸¦ ÅðÄ¡ÇÏ½Å°Å¿ä? ¼ÒÀÎÀÌ Á³±¸·Á.")
      ...
      NPC_SAY("{0xFFFFFF00}Èæ±ÍÇ÷·Î{END}¿¡ Ãâ¸ôÇÏ´Â {0xFFFFFF00}¼ö±Ý¾Æ{END} 130¸¶¸®¸¦ ´©°¡ ¸ÕÀú ÅðÄ¡ÇÏ´ÂÁö °æÀïÇØº¾½Ã´Ù.")
    end
  end
  if qData[2480].state == 1 then
    if qData[2480].killMonster[qt[2480].goal.killMonster[1].id] >= qt[2480].goal.killMonster[1].count then
      ...
      NPC_SAY("{0xFFFFFF00}Èæ±ÍÇ÷·Î{END}¿¡ Ãâ¸ôÇÏ´Â {0xFFFFFF00}ÅºÁÖ¾î{END} 150¸¶¸®¸¦ ´©°¡ ¸ÕÀú ÅðÄ¡ÇÏ´ÂÁö °æÀïÇØº¾½Ã´Ù.")
    end
  end
  if qData[2481].state == 1 then
    if qData[2481].killMonster[qt[2481].goal.killMonster[1].id] >= qt[2481].goal.killMonster[1].count then
      if 1 <= CHECK_INVENTORY_CNT(3) then
        NPC_SAY("¹ú½á ´Ù ÅðÄ¡ÇÏ¼Ì¼ÒÀÌ±î? À½.. ÀÏ´Ü Á» ½¬¾ú´Ù ´Ù½Ã ¸¸³³½Ã´Ù. {0xFFFFFF00}146°ø·Â{END}ÀÌ µÇ¸é ´Ù½Ã º¸µµ·Ï ÇÏÁÒ.")
      ...
      NPC_SAY("{0xFFFFFF00}Èæ±ÍÇ÷·Î{END}¿¡ Ãâ¸ôÇÏ´Â {0xFFFFFF00}»ç·æ{END} 120¸¶¸®¸¦ ´©°¡ ¸ÕÀú ÅðÄ¡ÇÏ´ÂÁö °æÀïÇØº¾½Ã´Ù.")
    end
  end
  if qData[2482].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2482].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(3) then
      ...
      NPC_SAY("{0xFFFFFF00}Èæ±ÍÇ÷·Î{END}¿¡ °¡¼­ {0xFFFFFF00}[¼ö±Ý¾Æ]{END}¸¦ ÅðÄ¡ÇÏ°í {0xFFFFFF00}Ä®³¯²¿¸®{END}¸¦ 100°³¸¦ ¸ÕÀú ±¸ÇØ¿À¸é ÀÌ±â´Â °Ì´Ï´Ù.")
    end
  end
  if qData[2483].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2483].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(3) then
      ...
      NPC_SAY("{0xFFFFFF00}Èæ±ÍÇ÷·Î{END}¿¡ °¡¼­ {0xFFFFFF00}[ÅºÁÖ¾î]{END}¸¦ ÅðÄ¡ÇÏ°í {0xFFFFFF00}ÅºÁÖ¾î»¡ÆÇ{END}À» 100°³¸¦ ¸ÕÀú ±¸ÇØ¿À¸é ÀÌ±â´Â °Ì´Ï´Ù.")
    end
  end
  if qData[2484].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2484].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(3) then
      ...
      NPC_SAY("{0xFFFFFF00}Èæ±ÍÇ÷·Î{END}¿¡ °¡¼­ {0xFFFFFF00}[»ç·æ]{END}¸¦ ÅðÄ¡ÇÏ°í {0xFFFFFF00}»ç·æ¼ö¿°{END}À» 100°³¸¦ ¸ÕÀú ±¸ÇØ¿À¸é ÀÌ±â´Â °Ì´Ï´Ù.")
    end
  end
  ...
  if qData[3681].state == 1 then
    if qData[3681].killMonster[qt[3681].goal.killMonster[1].id] >= qt[3681].goal.killMonster[1].count then
      ...
      NPC_SAY("ÊÇ¿´Ë­¸ü¿ìµÄ»÷ÍË{0xFFFFFF00}50¸öºÚ¹íÑªÂ·µÄÐ°Áú{END}µÄ¶Ô¾ö")
    end
  end
