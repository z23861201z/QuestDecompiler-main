...
function npcsay(id)
  ...
  if qData[1167].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1167].goal.getItem) and __QUEST_CHECK_ITEMS(qt[1167].goal.getItem) and __QUEST_CHECK_ITEMS(qt[1167].goal.getItem) then
  ...
  if qData[1172].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1172].goal.getItem) then
      NPC_SAY("谢谢…真的很感谢…")
