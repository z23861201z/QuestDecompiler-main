...
function npcsay(id)
  ...
  if qData[83].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[83].goal.getItem) then
      NPC_SAY("???? ??????. ??? ????? ???? ??? ?? ?????. ?????.")
  ...
  if qData[129].state == 1 and __QUEST_CHECK_ITEMS(qt[129].goal.getItem) and __QUEST_CHECK_ITEMS(qt[129].goal.getItem) then
    NPC_SAY("?. ??? ??? ??? ????. ??? ??? ??????. ?? ??? ???. ?? ????.")
  ...
  if qData[1188].state == 1 and __QUEST_CHECK_ITEMS(qt[1188].goal.getItem) and __QUEST_CHECK_ITEMS(qt[1188].goal.getItem) then
...
function chkQState(id)
  ...
  if qData[129].state == 1 and GET_PLAYER_LEVEL() >= qt[129].needLevel then
    if __QUEST_CHECK_ITEMS(qt[129].goal.getItem) and __QUEST_CHECK_ITEMS(qt[129].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
  ...
  if qData[1188].state ~= 2 and qData[1188].state == 1 and __QUEST_CHECK_ITEMS(qt[1188].goal.getItem) and __QUEST_CHECK_ITEMS(qt[1188].goal.getItem) then
    QSTATE(id, 2)
  end
  if qData[1228].state == 1 then
    QSTATE(id, 2)
