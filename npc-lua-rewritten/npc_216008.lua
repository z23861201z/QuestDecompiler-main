...
function npcsay(id)
  ...
  clickNPCid = id
  NPC_SAY("ÏÖÔÚÎÒÖ»ÓÐÕâÐ©ÁË£¬ÏëÂò¾ÍÂò°É¡£")
  if qData[1011].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1011].goal.getItem) then
      NPC_SAY("ÕæÌ«¸ÐÐ»ÄúµÄ°ïÃ¦ÁË¡£ÎÒÄÜ±¨´ðµÄ¾ÍÖ»ÓÐÕâÐ©ÁË")
  ...
  if qData[2420].state == 1 then
    NPC_SAY("¹è¸¦ °íÃÄ¾ß µË´Ï´Ù. È­Â÷·ûÀ» ÅðÄ¡ÇÏ°í {0xFFFFFF00}±×À»¸°³ª¹«Á¶°¢ 30°³{END}¸¦ ±¸ÇØ¼­ »ç°ø´Ô²² ÀüÇØµå¸®¼¼¿ä.")
  end
  if qData[2422].state == 1 then
    NPC_SAY("°¡Àå °¡±î¿î °÷Àº {0xFFFFFF00}°í¶ôÃÌÀÇ¿ø´ÔÀÌ °è½Ã´Â °í¶ôÃÌÀÔ´Ï´Ù.{END}")
  end
  if qData[2439].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2439].goal.getItem) and __QUEST_CHECK_ITEMS(qt[2439].goal.getItem) then
      NPC_SAY("°¨»çÇÕ´Ï´Ù.")
      SET_QUEST_STATE(2439, 2)
      return
    else
      NPC_SAY("±ÍÁÖµµ¿¡¼­¸¸ ³ª´Â ¹°°ÇµéÀÔ´Ï´Ù. {0xFFFFFF00}ÈæÇÏµ¿µÎ°³°ñ°ú ¿øÇü»ÀÁ¶°¢À» 10°³¾¿{END} ±¸ÇØÁÖ½Ã¸é µË´Ï´Ù. ÈæÇÏµ¿µÎ°³°ñÀº ÈæÇÏµ¿À», ¿øÇü»ÀÁ¶°¢Àº ÃËÀ½À» ÅðÄ¡ÇÏ°í ¾òÀ» ¼ö ÀÖ½À´Ï´Ù.")
    end
  end
  if qData[2440].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2440].goal.getItem) then
      NPC_SAY("°¨»çÇÕ´Ï´Ù.")
      SET_QUEST_STATE(2440, 2)
      return
    else
      NPC_SAY("±ÍÁÖµµ¿¡ °¡¼Å¼­ {0xFFFFFF00}ÈæÇÏµ¿µÎ°³°ñÀ» 70°³{END}¸¸ ±¸ÇØÁÖ¼¼¿ä. ÈæÇÏµ¿µÎ°³°ñÀº ÈæÇÏµ¿À» ÅðÄ¡ÇÏ°í ¾òÀ» ¼ö ÀÖ½À´Ï´Ù.")
    end
  end
  if qData[2441].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2441].goal.getItem) then
      NPC_SAY("°¨»çÇÕ´Ï´Ù. ±¸ÇØÁÖ½Å ±ÍÁÖµµÀÇ ¹°°ÇµéÀº Àß ¾²°Ú½À´Ï´Ù.")
      SET_QUEST_STATE(2441, 2)
      return
    else
      NPC_SAY("±ÍÁÖµµ¿¡ °¡¼Å¼­ ÃËÀ½À» ÅðÄ¡ÇÏ°í {0xFFFFFF00}¿øÇü»ÀÁ¶°¢ 70°³{END}¸¸ ±¸ÇØÁÖ¼¼¿ä.")
    end
  end
  ...
  if qData[2490].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2490].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(3) then
      ...
      NPC_SAY("{0xFFFFFF00}»ï°­µ¿Àüµî»ÔÀº Èæ±ÍÁÖ¿ø¿¡¼­ »ï°­µ¿ÀüÀ» ÅðÄ¡ÇÏ¸é{END} ±¸ÇÒ ¼ö ÀÖ½À´Ï´Ù. {0xFFFFFF00}»ï°­µ¿Àüµî»Ô 50°³{END}¸¦ ±¸ÇØÁÖ¼¼¿ä.")
    end
  end
  if qData[2491].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2491].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(4) then
        NPC_SAY("¾îµð.. ¸Â±º¿ä. Á¦°¡ ÁßÈ­½ÃÅ°°Ú½À´Ï´Ù.")
      ...
      NPC_SAY("{0xFFFFFF00}»çºÀ±Íµ¶Ä§Àº ¹é±ÍÁÖ¿ø¿¡¼­ »çºÀ±Í¸¦ ÅðÄ¡ÇÏ¸é{END} ±¸ÇÒ ¼ö ÀÖ½À´Ï´Ù. {0xFFFFFF00}»çºÀ±Íµ¶Ä§ 70°³{END}¸¦ ±¸ÇØÁÖ¼¼¿ä.")
    end
  end
  if qData[2492].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2492].goal.getItem) and __QUEST_CHECK_ITEMS(qt[2492].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(4) then
        NPC_SAY("¾îµð.. ¸Â±º¿ä.")
      ...
      NPC_SAY("{0xFFFFFF00}Èæ±ÍÁÖ¿ø¿¡¼­ À°¾È±Í{END}¸¦ ÅðÄ¡ÇÏ°í {0xFFFFFF00}À°¾È±Í¼ö¾× 50°³{END}¸¦, {0xFFFFFF00}Èæ±ÍÇ÷·Î¿¡¼­ ÅºÁÖ¾î{END}¸¦ ÅðÄ¡ÇÏ°í {0xFFFFFF00}ÅºÁÖ¾î»¡ÆÇ 50°³{END}¸¦ ±¸ÇØÁÖ¼¼¿ä.")
    end
  end
  if qData[2493].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2493].goal.getItem) then
      if 1 <= CHECK_INVENTORY_CNT(4) then
        NPC_SAY("ÁÁ½À´Ï´Ù. ÀÌÁ¦ ¸ðµç ÁØºñ°¡ ³¡³µ½À´Ï´Ù. ¿©±â {0xFFFFFF00}±¸±Þ¾à»óÀÚ{END}ÀÔ´Ï´Ù.")
        SET_QUEST_STATE(2493, 2)
      else
        NPC_SAY("Çà³¶ÀÌ ³Ê¹« ¹«°Ì½À´Ï´Ù!")
      end
    else
      NPC_SAY("{0xFFFFFF00}Èæ±ÍÇ÷·Î¿¡¼­ »ç·æ{END}À» ÅðÄ¡ÇÏ½Ã°í {0xFFFFFF00}»ç·æ¼ö¿°{END}À» 35°³¸¦ ±¸ÇØ¿À½Ã¸é µË´Ï´Ù.")
    end
  end
  if qData[3666].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[3666].goal.getItem) then
      NPC_SAY("Ð»Ð»£¡")
  ...
  if qData[3667].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[3667].goal.getItem) then
      NPC_SAY("Ð»Ð»£¡")
      SET_QUEST_STATE(3667, 2)
      return
    else
      NPC_SAY("°ïÃ¦ÊÕ¼¯{0xFFFFFF00}Öá»­ÑýÅ®{END}µÄ{0xFFFFFF00}60¸öÆÆ¾ÉÖá»­{END}»ØÀ´°É")
    end
  end
