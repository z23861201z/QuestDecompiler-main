...
function npcsay(id)
  ...
  if qData[1229].state == 1 and __QUEST_CHECK_ITEMS(qt[1229].goal.getItem) then
  ...
  if qData[1231].state == 1 and __QUEST_CHECK_ITEMS(qt[1231].goal.getItem) then
    NPC_SAY("好了！现在把这件事告诉冥珠城的武林人，那他们间的争斗也会缓和很多。")
    SET_QUEST_STATE(1231, 2)
  end
  if qData[1232].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1232].goal.getItem) then
  ...
  if qData[1239].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1239].goal.getItem) then
  ...
  if qData[2073].state == 1 and __QUEST_CHECK_ITEMS(qt[2073].goal.getItem) then
    SET_QUEST_STATE(2073, 2)
    NPC_SAY("欢迎光临！你就是PLAYERNAME啊~")
  end
  if qData[2074].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2074].goal.getItem) then
  ...
  if qData[2075].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2075].goal.getItem) then
  ...
  if qData[2077].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2077].goal.getItem) then
  ...
  if qData[2078].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[2078].goal.getItem) then
...
function chkQState(id)
  ...
  if qData[1231].state == 1 then
    if __QUEST_CHECK_ITEMS(qt[1231].goal.getItem) then
      QSTATE(id, 2)
    else
      QSTATE(id, 1)
    end
